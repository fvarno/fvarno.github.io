...
% \geometry{top=1in, bottom=1in, left=1in, right=1in} % Customize margins here
...
\begingroup
  ...
  \begin{tabular}{@{}l@{\hspace{1.8em}}l@{\hspace{1.8em}}l@{}}
    $if(phone)$\textbf{Phone:}~$phone$$else$\mbox{}$endif$ &
    $if(email)$\textbf{Email:}~$email$$else$\mbox{}$endif$ &
    $if(website)$\textbf{Website:}~\href{$website$}{$website$}$else$\mbox{}$endif$ \\
    $if(linkedin)$\textbf{LinkedIn:}~\href{$linkedin$}{$linkedin$}$else$\mbox{}$endif$ &
    $if(github)$\textbf{GitHub:}~\href{$github$}{$github$}$else$\mbox{}$endif$ &
...
\newcommand{\fhv}{\href{http://farshid.varnio.com}{Farshid Varno}}
...
% \footnotetext{This resume is compiled using \LaTeX\ and is also available in HTML format at \href{$footnoteurl$}{$footnoteurl$}.}
